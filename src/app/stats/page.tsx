...
const fetchStarterPacks = async () => {
  const { data, error } = await supabase
    .from('starter_packs')
    .select('*'); // Fetches id, name, description, card_data, etc.
...
const fetchDecks = async () => {
  ...
  if (decks && decks.length > 0) {
    ...
    console.log("No decks found for user. You might need to create a 'Main Deck' first.");
  }
};
...
const fetchProfile = async () => {
    const { data } = await supabase
      .from('profiles')
      .select('streak_count, blocked_words, auto_play_jp, auto_play_en, imported_packs')
      .eq('id', user?.id)
      .single();
...
const processWords = async (inputList: string[]) => {
  if (!user || !defaultDeckId) return alert("Please log in and ensure deck is initialized.");
  ...
  try {
    ...
    if (wordsForAI.length > 0) {
      try {
        ...
        alert(`Note: Added existing words, but new ones failed: ${aiErr.message}`);
      }
    }
...
const importPack = async (pack: any) => {
  if (!defaultDeckId) {
    alert("Deck ID is missing. Still loading your profile... please try again in 1 second.");
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && newBlockWord.trim() && (updateBlocklist([...userBlocklist, newBlockWord.trim()]), setNewBlockWord(""))}
                />
              </div>
            </div>
          </div>
  ...
  <div className="flex flex-col">
    {isEditingTitle ? (
      <div className="flex items-center gap-2">
        ...
        />
        <button onClick={updateDeckTitle} className="bg-emerald-500 text-white px-3 py-1 rounded-lg text-xs font-bold shadow-sm hover:bg-emerald-600 transition-colors">
          Save
        </button>
        <button onClick={() => setIsEditingTitle(false)} className="bg-slate-200 text-slate-600 px-3 py-1 rounded-lg text-xs font-bold hover:bg-slate-300 transition-colors">
          ✕
        </button>
      </div>
    ) : (
  ...
  <div className="flex flex-col md:flex-row gap-8 items-center">
    ...
    <div className="flex-1 w-full">
      <div className="flex items-center gap-2 mb-4">
        <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-400 text-[10px] font-black rounded-md border border-indigo-500/30 uppercase tracking-widest">Recognize</span>
        <p className="text-xs font-bold text-slate-400">🇯🇵 → 🇺🇸</p>
      </div>
      <div className="grid grid-cols-3 gap-2 text-center">
        <div>
          <p className="text-[10px] font-bold text-slate-500 uppercase">Tries</p>
          <p className="text-xl font-black">{globalStats.jp.tries}</p>
        </div>
        <div>
          <p className="text-[10px] font-bold text-indigo-400 uppercase">Accuracy</p>
          <p className="text-xl font-black">{globalStats.jp.tries > 0 ? Math.round((globalStats.jp.pass / globalStats.jp.tries) * 100) : 0}%</p>
        </div>
        <div>
          <p className="text-[10px] font-bold text-emerald-500 uppercase">Pass</p>
          <p className="text-xl font-black text-emerald-400">{globalStats.jp.pass}</p>
        </div>
      </div>
    </div>
    ...
    <div className="flex-1 w-full">
      <div className="flex items-center gap-2 mb-4">
        <span className="px-2 py-0.5 bg-orange-500/20 text-orange-400 text-[10px] font-black rounded-md border border-orange-500/30 uppercase tracking-widest">Recall</span>
        <p className="text-xs font-bold text-slate-400">🇺🇸 → 🇯🇵</p>
      </div>
      <div className="grid grid-cols-3 gap-2 text-center">
        <div>
        ...
        <div>
          <p className="text-[10px] font-bold text-orange-400 uppercase">Accuracy</p>
          <p className="text-xl font-black">{globalStats.en.tries > 0 ? Math.round((globalStats.en.pass / globalStats.en.tries) * 100) : 0}%</p>
        </div>
        <div>
          <p className="text-[10px] font-bold text-emerald-500 uppercase">Pass</p>
          <p className="text-xl font-black text-emerald-400">{globalStats.en.pass}</p>
        </div>
      </div>
    </div>
  ...
  return (
    <div 
      ...
      className={`flex-none w-64 p-6 rounded-[2rem] border-2 flex flex-col justify-between transition-all ${
        isOwned ? 'bg-slate-100 border-slate-200' : 'bg-white border-indigo-50 shadow-sm'
      }`}
    >
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed mb-6 line-clamp-2 min-h-[32px]">
              {pack.description || `${pack.card_data?.length || 0} essential words to kickstart your journey.`}
            </p>
          </div>
    ...
    {starterPacks.length === 0 && (
      <div className="flex-none w-full p-8 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center">
        <p className="text-sm text-slate-400 font-bold italic">Looking for new collections...</p>
      </div>
    )}
  </div>
</div>
        ...
        <div className="md:hidden space-y-4">
          {visibleCards.map((card) => (
            <div key={card.id} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm relative">
              <button onClick={() => deleteCard(card.id)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500">✕</button>
              <div className="mb-4">
              ...
              {visibleCards.map((card) => (
                <tr key={card.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => deleteCard(card.id)} className="opacity-0 group-hover:opacity-100 text-rose-400 hover:text-rose-600 font-bold text-sm">
                      Delete
                    </button>
                  </td>
                </tr>
