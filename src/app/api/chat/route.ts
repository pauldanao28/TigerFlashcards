...
natural_altは【ユーザーが書いた日本語のみ】を対象とする。ユーザーのメッセージをネイティブスピーカーが最も自然に言い直した一文を入れること（例：ユーザー「私は昨日映画を見に行きました」→ natural_alt「昨日映画見に行ったよ」）。ルール：①ユーザーが日本語で書いていない場合は必ず""にすること ②ユーザーの表現がすでに完全に自然な場合は必ず""にすること ③AIの返答・アドバイス・説明を絶対に入れないこと。`;
