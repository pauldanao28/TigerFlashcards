...
export default function ListeningQuiz({ userId, isAdmin = false, onClose }: ListeningQuizProps) {
  ...
  useEffect(() => {
    ...
    const target = Math.round((results.filter(r => r.gotIt).length / results.length) * 100);
    let current = 0;
        ...
        <div className="flex-1 flex flex-col items-center overflow-y-auto px-6 py-8 max-w-lg mx-auto w-full">
          ...
          >
            ...
            <p className="text-slate-400 font-black uppercase tracking-widest text-[10px] mt-1">
              {animatedPct}% caught by ear
            </p>
            {skillScore !== null && (
              <p className="text-sky-600 font-black text-2xl mt-3 tabular-nums">
                Listening Score <span className="text-4xl">{animatedScore}</span>%
              </p>
            )}
          </motion.div>
